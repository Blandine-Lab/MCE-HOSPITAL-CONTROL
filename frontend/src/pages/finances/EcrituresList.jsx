...
const EcrituresList = () => {
  ...
  const handleValider = (id) => {
    ...
    api.put(`/ecritures/${id}/valider`)
      .then(() => {
        setEcritures(ecritures.map(e => e.id === id ? { ...e, statut: 'validé' } : e));
  ...
  const handleAnnuler = (id) => {
    ...
    api.put(`/ecritures/${id}/annuler`)
      .then(() => {
        setEcritures(ecritures.map(e => e.id === id ? { ...e, statut: 'annulé' } : e));
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('⚠️ Supprimer définitivement cette écriture ? Seulement si elle est au statut "Brouillon". Cette action est irréversible.')) return;
    try {
              ...
              <tr key={e.id} style={{ borderBottom: i === ecritures.length - 1 ? 'none' : '1px solid #f1f5f9' }}>
                ...
                <td style={{ padding: '14px 20px' }}>
                  <span style={{
                    ...
                    fontSize: '12px',
                    fontWeight: '500',
                    backgroundColor: e.statut === 'validé' ? '#d1fae5' : e.statut === 'annulé' ? '#fee2e2' : '#fef3c7',
                    color: e.statut === 'validé' ? '#065f46' : e.statut === 'annulé' ? '#991b1b' : '#92400e'
                  }}>
                    {e.statut === 'validé' ? '✅ Validé' : e.statut === 'annulé' ? '❌ Annulé' : '📝 Brouillon'}
                  </span>
                </td>
                <td style={{ padding: '14px 20px', textAlign: 'center' }}>
                  ...
                  {(e.statut === 'brouillon' || e.statut === 'validé') && (
