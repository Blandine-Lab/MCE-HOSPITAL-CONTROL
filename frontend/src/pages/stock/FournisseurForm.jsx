// src/pages/stock/FournisseurForm.jsx
...
const FournisseurForm = () => {
  ...
  const isEdit = !!id;
  const [formData, setFormData] = useState({ nom: '', contact: '', telephone: '', email: '', adresse: '' });
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  useEffect(() => {
    if (isEdit) {
      api.get(`/fournisseurs/${id}`).then(res => setFormData(res.data)).catch(console.error);
    }
  }, [id, isEdit]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) await api.put(`/fournisseurs/${id}`, formData);
      else await api.post('/fournisseurs', formData);
      navigate('/stock/fournisseurs');
    } catch (err) { 
      setError(err.response?.data?.error || 'Erreur lors de l\'enregistrement du fournisseur'); 
      setLoading(false); 
    }
  };
  ...
  return (
    <div>
      <Link to="/stock/fournisseurs" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', color: '#3b82f6', textDecoration: 'none' }}><FaArrowLeft /> Retour</Link>
