...
const Admin = () => {
  ...
  // ===== NOUVEAUX ÉTATS POUR LA MODIFICATION DES MÉDECINS =====
  const [editMedecinId, setEditMedecinId] = useState(null);
  const [editMedecinData, setEditMedecinData] = useState({
    nom: '', prenom: '', specialite: '', email: '', login: '', password: ''
  });
  ...
  const handleEditMedecin = (med) => {
    setEditMedecinId(med.id);
    setEditMedecinData({
      nom: med.nom || '',
      prenom: med.prenom || '',
    ...
    setShowMedecinModal(true);
  };

  // Soumettre la modification
  const handleUpdateMedecin = async (e) => {
    e.preventDefault();
    if (!editMedecinData.nom || !editMedecinData.prenom) {
      showToast('Nom et prénom sont requis', 'error');
      return;
    }
    try {
      const payload = { ...editMedecinData };
      if (!payload.password) delete payload.password;
      await api.put(`/consultations/medecins/${editMedecinId}`, payload);
      setShowMedecinModal(false);
      setEditMedecinId(null);
      fetchData();
      showToast('Médecin modifié avec succès');
    } catch (err) {
      showToast(err.response?.data?.error || 'Erreur', 'error');
    }
  };
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', flexWrap: 'wrap' }}>
        ...
        <button onClick={() => setActiveTab('dispositifs')} style={{ padding: '8px 16px', backgroundColor: activeTab === 'dispositifs' ? '#3b82f6' : '#e5e7eb', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>🛠️ Dispositifs</button>
      ...
      {/* MÉDECINS (avec modification et suppression) */}
      {activeTab === 'medecins' && (
        <div style={{ border: '1px solid #ddd', padding: '20px', borderRadius: '8px', backgroundColor: '#fff' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr 1fr auto', gap: '10px', marginBottom: '20px' }}>
            ...
            <input type="text" placeholder="Login (optionnel)" value={newMedecin.login} onChange={e => setNewMedecin({ ...newMedecin, login: e.target.value })} style={{ padding: '8px', border: '1px solid #ccc', borderRadius: '4px' }} />
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#3b82f6', color: 'white' }}>
                ...
                <th style={{ padding: '8px', border: '1px solid #ddd' }}>Spécialité</th>
            ...
            <tbody>
              {medecins.map(m => (
                <tr key={m.id}>
                  ...
                  <td style={{ padding: '8px', border: '1px solid #ddd' }}>{m.prenom}</td>
                  <td style={{ padding: '8px', border: '1px solid #ddd' }}>{m.specialite}</td>
                  <td style={{ padding: '8px', border: '1px solid #ddd' }}>{m.email}</td>
                  <td style={{ padding: '8px', border: '1px solid #ddd' }}>
                    <button
                      onClick={() => handleEditMedecin(m)}
                      style={{ marginRight: '8px', background: '#eab308', border: 'none', padding: '4px 8px', borderRadius: '4px', cursor: 'pointer' }}
                    >
                      <FaEdit /> Modifier
                    </button>
                ...
                <tbody>
                  {medicaments.map(m => (
                    <tr key={m.id}>
                      ...
                      <td style={{ padding: '8px', borderBottom: '1px solid #ddd' }}>{m.est_stupefiant ? '⚠️' : '✅'}</td>
      ...
      {showMedecinModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
          <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '24px', width: '500px', maxWidth: '90%' }}>
            <h2 style={{ marginBottom: '16px' }}>Modifier le médecin</h2>
            <form onSubmit={handleUpdateMedecin}>
              <div style={{ marginBottom: '12px' }}>
                <label>Nom *</label>
                <input type="text" value={editMedecinData.nom} onChange={e => setEditMedecinData({...editMedecinData, nom: e.target.value})} required style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '6px' }} />
              </div>
              <div style={{ marginBottom: '12px' }}>
                <label>Prénom *</label>
                <input type="text" value={editMedecinData.prenom} onChange={e => setEditMedecinData({...editMedecinData, prenom: e.target.value})} required style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '6px' }} />
              </div>
              <div style={{ marginBottom: '12px' }}>
                <label>Spécialité</label>
                <input type="text" value={editMedecinData.specialite} onChange={e => setEditMedecinData({...editMedecinData, specialite: e.target.value})} style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '6px' }} />
              </div>
              <div style={{ marginBottom: '12px' }}>
                <label>Email</label>
                <input type="email" value={editMedecinData.email} onChange={e => setEditMedecinData({...editMedecinData, email: e.target.value})} style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '6px' }} />
              </div>
              <div style={{ marginBottom: '12px' }}>
                <label>Login</label>
                <input type="text" value={editMedecinData.login} onChange={e => setEditMedecinData({...editMedecinData, login: e.target.value})} style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '6px' }} />
              </div>
              <div style={{ marginBottom: '20px' }}>
                <label>Nouveau mot de passe (laisser vide pour ne pas changer)</label>
                <input type="password" value={editMedecinData.password} onChange={e => setEditMedecinData({...editMedecinData, password: e.target.value})} style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: '6px' }} />
              </div>
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                <button type="button" onClick={() => setShowMedecinModal(false)} style={{ backgroundColor: '#e5e7eb', padding: '8px 16px', border: 'none', borderRadius: '6px', cursor: 'pointer' }}><FaTimes /> Annuler</button>
                <button type="submit" style={{ backgroundColor: '#16a34a', color: 'white', padding: '8px 16px', border: 'none', borderRadius: '6px', cursor: 'pointer' }}><FaSave /> Enregistrer</button>
              </div>
            </form>
          </div>
