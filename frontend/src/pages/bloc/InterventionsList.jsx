...
const InterventionsList = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('✅ Confirmer l\'annulation de cette intervention ? Cette action est irréversible.')) {
      try {
        ...
        if (err.response?.status === 403) {
          setToast('⚠️ Seul un administrateur peut annuler/supprimer une intervention.');
        } else {
    ...
    <div style={cardStyle}>
      {toast && (
        <div style={{
          ...
          backgroundColor: toast.includes('Erreur') || toast.includes('⚠️') ? '#ef4444' : '#10b981',
          color: 'white',
          padding: '12px 24px',
          borderRadius: '8px',
